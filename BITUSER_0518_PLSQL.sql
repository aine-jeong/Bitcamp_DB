...
--하나의 블럭으로 묶어서 실행시켜야한다.
DECLARE --선언
  vno number(4); --vno: 변수 / number: 타입
  vname varchar2(20); --vname: 변수
BEGIN --실행
  vno := 100; -- 할당 >  String s; s = "홍길동" / := assign
  vname := 'kglim';
...
BEGIN
   select empno ,ename
      into vno , vname --pl-sql 사용하는 구분 (into) . 실행결과를 [ 변수에 담기 ]
   from emp
